...
const targetOldBlock = `      sheet.innerHTML = \`
...
const targetNewBlock = `      sheet.innerHTML = \`
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
...
if (code.includes(targetOldBlock)) {
  code = code.replace(targetOldBlock, targetNewBlock);
  fs.writeFileSync('custom/extension.js', code);
  console.log('Replaced successfully');
} else {
  console.log('Failed to find block. Attempting regex.');
  const regex = /sheet\.innerHTML\s*=\s*`[\s\S]*?`;/m;
  code = code.replace(regex, targetNewBlock);
  fs.writeFileSync('custom/extension.js', code);
